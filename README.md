# Chess

> Final Project in TOP Ruby Course. Command line Chess game for 2 players to play against each other.

---

# About This Repository

This repository is part of my preparation for participating in the **[RoleModel Software Craftsmanship Academy – 2026 Cohort](https://rolemodelsoftware.com/academy)**.

This project is part of the pre-work I am completing through modules primarily from **The Odin Project**. Each assignment and project has its own repository for simplicity, readability, and because some projects can be deployed using **GitHub Pages**.

---

# The Learning Source

This project follows a lesson from **[The Odin Project](https://www.theodinproject.com/about)**.

**Lesson:**
[Chess](https://www.theodinproject.com/lessons/ruby-chess)

The Odin Project provides a comprehensive open-source curriculum focused on learning web development through hands-on project building.

---

# Project Overview

* Ruby Classes, and Modules
* Ruby Testing with RSpec
* TDD practice

---

# Skills Practiced

* Ruby Classes
* Ruby Methods
* Ruby Modules
* Ruby Marshal (save/load game state)
* RSpec testing
* TDD (mostly)
* 

---

# Running the Project Locally

Clone the repository:

```bash
git clone https://github.com/Silas-N-Schlax/rm-prework-odin-chess.git
```

Navigate to the project directory:

```bash
cd rm-prework-odin-chess
```

Bundle
```bash
bundle install
```

Run `ruby chess.rb` in your terminal.

---

# How to Use the Program

Run `ruby chess.rb` to start the program.

* Type `load` or `l` to load a saved game.
* Type `save` or `s` to save a game.
* Type `exit` or `e` to end the game at any time.

## Input and how to play

Input is weird, my game is like a toddler, it likes to tell you no, but they refused to tell you why...
The easy thing to remember is you have two required inputs seperated by a comma. The first is the starting
piece, the second is the destination you want it.

### Example: (assume starting board on black's first move)

You (black) want to move your left Knight on `[2, 1]` to `[1, 3]`
Good, you got that far, now you have to remove some characters and spaces and oyu will be good.

So valid input would look like `21,13`.

The computer then takes that and does the following translations. *Yes I know is backwards, I didn't realize till it was too late...*

1. `['21', '13]` - Split at the comma.
2. `[['2', '1'], ['1', '3']]` - Split each group.
3. `[[2, 1], [1, 3]]` - Switch to number (to_i).
4. `[[1, 2], [3, 1]]` - Flip [x, y] to [y, x] (*I TOLD YOU I KNOW!*).
5. `[[0, 1], [2, 0]]` - Subtract 1 for indexing.

And then the computers does more with it and tells moves the piece if its valid (*hint, this one is*;)

![Picture of board at starting positions](/board_design.png)
---

# Reflection

Key things learned from building this project:

* TDD is annoying but very helpful in complext projects.
* Marshal saves object state over using JSON which would need manual repopulation.
* 

Things I would improve in a future iteration:

* Add the more obcure rules of Chess.
* Better UI that is readable...

---

# Commit Philosophy

This repository follows a workflow of **small, descriptive commits** to document the development process and learning progression. I aim to practice **atomic commits** whenever possible so that each commit reflects a clear and focused change.

---

# License

This project is released under the **Creative Commons Zero v1.0 Universal (CC0 1.0)** license.

