...
RSpec.describe Pawn do
  ...
  describe '#valid_move' do
    ...
    context 'when black pawn moves' do
      let(:pawn) { described_class.new('b', [1, 0]) }
      context 'returns true' do
        it 'when space is [2, 0]' do
          expect(pawn.valid_move?(board, [2, 0])).to be true
        end
        it 'when space is [3, 0] and pawn is in starting position' do
          expect(pawn.valid_move?(board, [3, 0])).to be true
        end
        it 'when space is [2, 1] and capture' do
          board.board[2][1] = Pawn.new('w', [2, 1])
          expect(pawn.valid_move?(board, [2, 1])).to be true
        end
      end
      context 'returns false' do
        it 'when space is [0, 0]' do
          expect(pawn.valid_move?(board, [0, 0])).to be false
        end
        it 'when space is [0, 0] and pawn has been removed' do
          board.board[0][0] = nil
          expect(pawn.valid_move?(board, [0, 0])).to be false
        end
        it 'when space is [4, 0]' do
          expect(pawn.valid_move?(board, [4, 0])).to be false
        end
        it 'when space is [2, 1] and no capture' do
          expect(pawn.valid_move?(board, [2, 1])).to be false
        end
        ...
        it 'when space is [3, 2] (out of bound capture attempt)' do
          expect(pawn.valid_move?(board, [3, 2])).to be false
        end
        it 'when space is [0, 1]' do
          expect(pawn.valid_move?(board, [0, 1])).to be false
        end
        ...
        it 'when space is [1, 2]' do
          expect(pawn.valid_move?(board, [1, 2])).to be false
        end
    ...
    context 'when white pawn moves' do
      let(:pawn) { described_class.new('w', [6, 0]) }
      context 'returns true' do
        it 'when space is [5, 0]' do
          expect(pawn.valid_move?(board, [5, 0])).to be true
        end
        it 'when space is [4, 0] and pawn is in starting position' do
          expect(pawn.valid_move?(board, [4, 0])).to be true
        end
        it 'when space is [5, 1] and capture' do
          board.board[5][1] = Pawn.new('b', [5, 1])
          expect(pawn.valid_move?(board, [5, 1])).to be true
        end
      end
      context 'returns false' do
        it 'when space is [7, 0]' do
          expect(pawn.valid_move?(board, [7, 0])).to be false
        end
        it 'when space is [7, 0] and pawn has been removed' do
          board.board[7][0] = nil
          expect(pawn.valid_move?(board, [7, 0])).to be false
        end
        it 'when space is [3, 0]' do
          expect(pawn.valid_move?(board, [3, 0])).to be false
        end
        it 'when space is [5, 1] and no capture' do
          expect(pawn.valid_move?(board, [5, 1])).to be false
        end
        it 'when space is [4, 2] (out of bound capture attempt)' do
          expect(pawn.valid_move?(board, [4, 2])).to be false
        end
        ...
        it 'when space is [7, 1]' do
          expect(pawn.valid_move?(board, [7, 1])).to be false
        end
        it 'when space is [0, 1] and space is empty' do
          board.board[7][1] = nil
          expect(pawn.valid_move?(board, [7, 1])).to be false
